...
- [ ] "예매 내역" 화면에는 사용자가 예매한 영화 목록을 보여준다.
- [ ] "설정" 화면은 기본적으로 알림 기능 On/Off 설정을 제공하며, 초기에는 빈 화면이다.
- [ ] 예매 내역 화면에서 예매한 영화 목록이 리스트 형태로 표시된다.
- [ ] 리스트 항목을 터치하면 해당 영화의 예매 상세 정보 화면으로 이동한다.
- [ ] 리스트 항목을 터치했을 때 시각적 효과(선택 효과 등)가 적용된다. (디자인 시안 참고)
- [ ] 앱을 재실행해도 기존 예매 내역이 유지된다.
- [ ] 설정 화면에서 알림 기능을 On/Off로 설정할 수 있다.
- [ ] 앱을 재실행해도 알림 설정 값이 유지된다.
- [ ] 영화 시작 30분 전에 푸시 알림이 전송된다.
- [ ] 푸시 알림을 클릭하면 해당 영화의 예매 상세 정보 화면으로 이동한다.