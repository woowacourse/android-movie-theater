 # android-movie-theater

## 1단계 기능 요구 사항
- 영화 선택 후 극장을 선택할 수 있다.
- 예매 완료 화면에서 극장 이름이 길 경우 말줄임표로 표시되어야 한다. (시안 참고)
- 극장별로 상영하는 영화와 상영시간은 달라질 수 있다.
- 예를 들어 시간표를 다음처럼 구성할 수 있다.
```
선릉 극장
- 스타워즈(09시, 11시, 15시)
- 해리포터(13시, 15시, 17시, 19시)

잠실 극장
- 스즈메의 문단속(09시)
- 해리포터(10시, 12시, 14시)
- 범죄도시(11시, 13시)
```
  
## 1단계 구현할 기능 목록
- [x] 예매 버튼을 누르면 극장 선택 창이 뜬다.
- 극장 선택 화면
- [x] 극장 선택 화면(xml), item(xml) 구현
- [x] 해당 영화를 상영하는 극장이 보여야 한다.
- [x] 각 극장은 선택된 영화의 총 상영 시간의 개수가 보여야 한다.
- [x] 극장을 선택하면 영화 예매 화면으로 넘어간다.
- 예매 완료 화면
- [X] 예매 완료 화면(xml)에 극장 이름 추가
- [X] 상영 극장 이름을 화면에 출력한다.
- [X] 너무 긴 극장 이름은 줄임표를 사용하여 출력한다.
- 영화관
- [x] 영화관은 이름이랑 상영할 수 있는 영화 목록과 상영시간표를 가진다.
- [x] 영화관은 총 상영 개수를 반환할 수 있다.
- [x] 해당 영화 제목으로 상영시간표를 반환할 수 있다.
- 영화관들 (Theaters)
- [X] 영화관들(Theaters)는 모든 영화관을 가진다.
- [X] 영화관들은 해당 영화를 상영하는 영화관 리스트를 반환할 수 있다.

- 영화 메인 페이지 
  - [x] 데이터 바인딩
  - [X] drawable 리소스를 위한 데이터 바인딩 어댑터 구현
- [x] 극장 선택 화면 데이터 바인딩
- [x] 영화 예매 페이지 데이터 바인딩
- [x] 영화 좌석 페이지 데이터 바인딩
- 영화 예매 완료 페이지 
  - [x] 데이터 바인딩
  - [X] 영화 예매 시간 포맷을 위한 데이터 바인딩 어댑터 구현

## 2단계 기능 요구 사항
- 영화 예매 내역, 홈, 설정 화면으로 이동(navigate)할 수 있다.
  - 예매 내역 : 빈 화면 
  - 홈 : 영화 목록 
  - 설정 : 빈 화면

## 2단계 구현할 기능 목록
- [X] 예매 화면 Fragment 구현
- [X] 홈 화면 Fragment 구현
- [X] 설정 화면 Fragment 구현
- [X] bottomNavigationBar 구현
- [X] MovieActivity 구현

## 리팩토링할 부분
- [X] 좌석 선택 화면에서 인원 수 상관 없이 모두 선택되던 현상 수정

# 말리빈 리뷰 반영
- [X] xml id도 축약어 지양
- [X] Fragment 재생성 지양
- [X] 예약된 곳에 예약을 다시 누르는 것은 정말로 예측 불가능한 "오류"인지 고민
- [X] Movie 도메인 고민
- [X] BindingAdapter 필요한 이유
- [X] LayoutManager xml에서 설정
- [X] MoviePresenter 테스트 가능하게 변경
- [X] 하드코딩된 값 상수화

## 3단계 구현할 기능 목록
- [ 화면1 ] 예매 내역
  - [X] 영화 예매 내역에는 날짜, 시간, 극장, 영화 제목이 있다.
  - [X] 영화 예매 내역을 누르면, 회색으로 변한다. (누르는 효과)
  - [X] 영화 예매 내역을 누를 수 있다.
  - [X] 영화 예매 내역을 누르면 예매한 영화 완료 화면을 보여준다.
  - [X] 앱을 재실행해도 기존 예매 내역이 유지되어야 한다.
    - [X] 기존 예매 내역을 데이터 베이스에 입력하는 로직
    - [X] 기존 예매 내역을 불러오는 로직
- [ 화면2 ] 예매 완료 화면
  - [X] 예매 완료 화면에서 뒤로 가기가 있다.
  - [X] 예매 완료 화면에서 뒤로 가기를 누를 수 있다.
  - [X] 영화 예매 완료 화면에서 뒤로 가기를 누르면 예매 내역으로 이동한다.

## 해결하고 싶은 부분
- [ ] 영화 예매 내역을 누르면, 회색으로 변한다. (누르는 효과)가 테스트가 불가능한 코드가 되었다. -> 개선방안 생각
- [ ]  ReservationViewHolder에서 position에 -1이라는 의미 없는 값을 넣어둠에 대해서 어떻게 생각하는 지
- [X] 예매 내역에서 데이터를 불러오고 화면를 띄우게 변화

## 4단계 구현할 기능 목록
- [ 화면1 ] 설정
  - [X] 알람 기능을 on/off할 수 있는 화면이 있다.
  - [X] 알람 기능을 on/off할 수 있다.
  - [X] 사용자가 앱을 재실행해도 설정 데이터가 남아있다.
- [X] 알람 권한을 설정할 수 있다.
- [X] 영화 시작 시간 30분 전에 푸시 알림이 온다
- [X] 푸시 알림을 클릭하면, 예매 완료 화면를 보여준다.

## 말리빈 리뷰 반영 2차
- [ ] Fragment에 데이터를 넘겨주는 방법
- [ ] 데이터 바인딩 가져오는 방법 - DataBindingUtil와 다른 것 찾아보기
- [ ] 뷰바인딩에서는 데이터바인딩의 무언가를 사용할 수 없는 것을 알아보기
- [ ] Movie 뷰와 도메인의 분리
- [ ] xml에서 DataBinding을 옹호하는 입장 되어보기
- [ ] Fragment의 생명주기
- [X] ItemId가 Fragment의 ID를 의미하는 지 알 수 없음 -> ItmeId 클래스명 수정
- [X] BookingStatus 생성자를 사용하지 않고 invoke 연산자를 오버로딩한 이유는 무엇인가요?
- [X] 외부에서 BookingSeats의 _seats(mutableList)에 접근하지 못하도록 깊은 복사 실행 
- 고민할 내용
  - [ ] Fragment로만 탭을 구현할 수 있는가? Fragment의 생명주기는 매우 복잡하고 다루기 어려우니, 그냥 통짜 View를 사용하면 되는 것 아닌가?
