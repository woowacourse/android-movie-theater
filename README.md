...
### 미션 3단계 요구사항
- [x] RoomDatabase를 구현한다.
- [x] 영화 에매 티켓을 데이터베이스에 저장할 수 있다.
- [x] 영화 내역을 예매 내역 화면에서 확인할 수 있다.
- [x] 알림 설정 화면을 구현한다.
- [x] 에메 내역을 터치하면 예매 정보를 보여준다.
- [x] 리스트 항목을 누르는 효과를 줄 수 있다. 
- [x] 앱을 실행해도 기존의 예매 내역이 유지된다.

### 미션 4단계 요구사항
- [x] 설정에서 알림 기능을 On/Off 할 수 있다.
- [x] 사용자가 앰을 재실행해도 설정 데이터가 남아있다. 
- [x] 영화 시작 시작 30분 전에 푸시 알림이 온다.
  - [x] 테스트용: 영화를 예매하면 15초 후에 알림이 뜬다.
- [x] 푸시 알림을 누르면 알림이 사라지고, 영화 예매 정보를 보여준다.
- [x] 푸시 알림의 icon을 커스텀할 수 있다. 
- [x] 알림 권한 및 Notification은 기본 Ui를 사용해서 구현한다.
- [x] 사용자가 알림 권한을 허용하도록 알림을 계속 띄운다.