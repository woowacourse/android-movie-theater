...
class ScreeningDateTimesGenerator(startDate: LocalDate, endDate: LocalDate) { // 기초 미션 코드 분리
    private val _times: Map<LocalDate, List<LocalTime>> =
        makeDates(startDate, endDate).associateWith { date -> makeTimes(date) }
    ...
    private fun makeTimes(date: LocalDate): List<LocalTime> {
        ...
        return (startHour until MAX_HOUR step STEP).map {
            LocalTime.of(it, MINUTE_DEFAULT)
        }
    }
