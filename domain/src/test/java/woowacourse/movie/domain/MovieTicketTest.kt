...
class MovieTicketTest {
    ...
    @Test
    fun `티켓 금액이 26000원일 때 조조 영화인 경우 예매 금액은 22000원이다`() {
        ...
        val ticket = MovieTicket(
            ...
            PeopleCount(2),
            setOf(seatFirst, seatSecond),
            Price(26000)
        )

        assertThat(ticket.getDiscountPrice().amount).isEqualTo(22000)
    }

    @Test
    fun `티켓 금액이 26000원일 때 심야 영화인 경우 예매 금액은 22000원이다`() {
        val seatFirst = Seat(1, 1, Rank.A)
        val seatSecond = Seat(1, 2, Rank.A)
        val ticket = MovieTicket(
            "title",
    ...
    @Test
    fun `티켓 금액이 26000원일 때 무비 데이고 조조 영화인 경우 예매 금액은 19400원이다`() {
        val seatFirst = Seat(1, 1, Rank.A)
        val seatSecond = Seat(1, 2, Rank.A)
        val ticket = MovieTicket(
            "title",
        ...
        assertThat(ticket.getDiscountPrice().amount).isEqualTo(19400)
    }
    ...
    fun `조조 영화일 때 A, B, S 등급 좌석을 선택하고 B 등급 좌석을 취소하면 티켓 가격은 21000원이다`() {
        ...
        assertThat(ticket.getDiscountPrice().amount).isEqualTo(21000)
    }
    ...
    fun `무비데이 이고 조조 영화일 때 A, B, S 등급 좌석을 선택하고 B 등급 좌석을 취소하면 티켓 가격은 18300원이다`() {
        ...
        assertThat(ticket.getDiscountPrice().amount).isEqualTo(18300)
    }
}
