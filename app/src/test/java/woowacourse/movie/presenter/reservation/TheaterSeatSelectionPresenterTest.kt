...
class TheaterSeatSelectionPresenterTest {
    ...
    fun `좌석 금액이 25000인 상황에서 A좌석을 선택하면 총 결제 금액을 37000으로 업데이트하여 화면에 표시한다`() {
        ...
        val expectedPrice = seatSelection.calculateAmount() + Grade.A.price
    ...
    fun `화면 회전이 발생했을 때 선택되었던 좌석들을 복구하여 화면에 표시되어야 한다`() {
        ...
        every { view.restoreSelectedSeats(seatSelection.seatsIndex) } answers {
            assertEquals(
                expectedSeatIndex, seatSelection.seatsIndex,
            )
        }
        presenter.restoreSeats(seatSelection, seatSelection.seatsIndex)
        verify { view.restoreSelectedSeats(seatSelection.seatsIndex) }
    }
    ...
    fun `화면 회전이 발생했을 때 예약 상태를 복구하여 화면에 표시되어야 한다`() {
        ...
        every { view.showAmount(seatSelection.calculateAmount()) } answers {
            val actualPrice = arg<Int>(0)
            assertEquals(expectedPrice, actualPrice)
        }
        presenter.restoreReservation(3)
        verify { view.setConfirmButtonEnabled(any()) }
        verify { view.showAmount(seatSelection.calculateAmount()) }
    }
}
