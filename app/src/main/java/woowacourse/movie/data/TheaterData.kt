...
object TheaterData {
    private val values = mutableListOf<TheaterEntity>(
        TheaterEntity(
            "잠실극장", listOf(
                MovieEntity(
            ...
            "선릉극장", listOf(
                MovieEntity(
                    ...
                    LocalDate.of(2023, 4, 15),
                    LocalDate.of(2023, 5, 2),
                    listOf("10:00", "12:00", "13:00", "16:00"),
                ), MovieEntity(
                    R.drawable.harry_potter,
                    "해리포터 - 2편",
                    "2023.04.11",
    ...
    fun getMovieSchedule(movieName: String): List<List<String>> {
        return values
            .flatMap { it.movies }
            .filter { it.title == movieName }
            .map { it.movieSchedule }
    }

    fun findByMovieName(movieName: String): List<TheaterEntity> {
        return values.filter { theaterEntity ->
            theaterEntity.movies.any {
                it.title == movieName
            }
        }
    }
